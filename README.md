...
Un **modelo** es un conjunto de reglas que el ordenador deduce de los datos. Nadie se las dicta. Aquí nadie escribió que los contactos llegados por recomendación de otro cliente valgan mucho: la máquina lo encontró sola. **Entrenar** es el proceso por el que esas reglas se ajustan mirando casos con la respuesta ya conocida, y el programa propone, mide su error y se corrige cientos de veces seguidas.
...
2. Las palabras se vuelven números, porque un ordenador no compara `Chat` con `Google`: cada respuesta posible pasa a ser una columna que vale 0 o 1.
...
Los dos montones se separan antes de tocar nada, y el reparto es **estratificado**: ambos conservan la misma proporción de compradores, el 37,6 %. Así el examen no sale más fácil ni más difícil que la clase, y con el primer montón se ajusta el modelo mientras que con el segundo, que no ha visto nunca, se le toma la lección.
...
**AUC** son las iniciales de una medida que puntúa lo bien ordenada que queda la lista. Se lee sin fórmula ninguna. Coge un comprador al azar y un no comprador al azar, mira si el modelo le puso más nota al comprador, y el AUC es la proporción de veces que gana esa comparación. Sorteando 400.000 parejas del conjunto de prueba, el comprador iba por delante en 92 de cada 100.
...
Call by call: out of every hundred in that band, 93 land on someone who ended up buying, against 38 for calls made at random. The value of this work lies there. In the allocation of sales effort. It counts for more than the model's hit rate on its own.
...
A **model** is a set of rules the computer works out from the data, with nobody dictating them. Nothing here says that contacts arriving through a referral are worth a lot: the machine found that on its own. **Training** is the process by which those rules are tuned by looking at cases whose answer is already known, and the program proposes, measures its error and corrects itself hundreds of times over.
...
2. Words become numbers, because a computer cannot compare `Chat` with `Google`: every possible answer becomes a column holding 0 or 1.
...
The two piles are separated before anything else is touched, and the split is **stratified**: both keep the same share of buyers, 37.6%. So the exam is neither easier nor harder than the class, and the first pile tunes the model while the second one, which the model has never seen, is the one that tests it.
...
**AUC** stands for a measure that scores how well the list ends up sorted. It reads without any formula at all. Take a buyer at random and a non-buyer at random, see whether the model gave the buyer the higher score, and the AUC is the proportion of times that comparison goes the right way. Drawing 400,000 such pairs from the test set, the buyer came out ahead in 92 out of 100.
